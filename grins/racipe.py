...
import argparse
import pandas as pd
from grins.generate_params import parse_topos, gen_param_names, gen_param_df, get_param_range_df, gen_init_cond
...
def main():
    ...
    # parser.add_argument('--genprs', type=bool, help='generate parameter range file, no simulation', default=False)
    ...
    # os.environ['XLA_FLAGS'] = f'--xla_force_host_platform_device_count={args.num_cores}'
    ...
    for tpfl in topos:
        ...
        # Switch cases for cfg, prs tbd
        default(topo_name, tpfl, args)
     
def default(topo_name, tpfl, args):
    ...
    prange_df.to_csv(f"{args.outdir}/{topo_name}/{topo_name}.prs", index=False, sep='\t')
    # Generate the parameter dataframe
    param_df = gen_param_df(prange_df, args.num_paras)
    param_df.to_csv(f"{args.outdir}/{topo_name}/{topo_name}_params.dat", sep='\t', index=True)
    # Generate the initial conditions dataframe
    init_df = gen_init_cond(topo_df, args.num_inits)
